...
const preflight = await request('/proxy/google/v1beta/openai/chat/completions', {
  method: 'OPTIONS',
  headers: {
    Origin: 'https://app.example.test',
...
const blockedGeminiOperation = await request('/proxy/google/v1beta/models/gemini-3-pro:delete', {
  method: 'POST',
...
await withMockedFetch(async (input, init) => {
  ...
  const response = await request('/proxy/google/v1beta/openai/chat/completions?alt=sse', {
    method: 'POST',
...
await withMockedFetch(async (input, init) => {
  ...
  const response = await request('/proxy/google/v1beta/models?pageSize=1000', {
    method: 'GET',
    headers: geminiAccessHeaders,
  })
  assert.equal(response.status, 200)
...
await withMockedFetch(async (input, init) => {
  ...
  const response = await request('/proxy/google/v1beta/models/gemini-3-pro:generateContent', {
    method: 'POST',
    headers: geminiAccessHeaders,
...
await withMockedFetch(async (input, init) => {
  ...
  const response = await request('/proxy/google/v1beta/models/gemini-3-pro:streamGenerateContent?alt=sse', {
    method: 'POST',
    headers: geminiAccessHeaders,
    body: '{}',
  })
