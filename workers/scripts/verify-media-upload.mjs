...
const plain = await request(mediaPath)
assert.equal(plain.status, 200)
assert.equal(Number(plain.headers.get('content-length')), file.size)
assert.equal(plain.headers.get('content-range'), null)
assert.equal((await plain.arrayBuffer()).byteLength, file.size)
