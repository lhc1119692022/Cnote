...
const GenerationChannelsManager = lazy(() => import('@/components/settings/GenerationChannelsManager').then((module) => ({ default: module.GenerationChannelsManager })))
    ...
    <DesktopWindowChrome>
      <Suspense fallback={<div className="flex h-full items-center justify-center text-sm text-muted-foreground">正在加载...</div>}>
        <Routes>
          ...
          <Route path="/settings/generation-channels" element={<GenerationChannelsManager />} />
        </Routes>
      </Suspense>
    </DesktopWindowChrome>
  )
