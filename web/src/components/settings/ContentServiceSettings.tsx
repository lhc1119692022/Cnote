  ...
  return (
    <section>
      <div className="mb-5 flex items-start gap-2.5 rounded-lg bg-muted/55 px-3.5 py-3 text-[12px] leading-relaxed text-muted-foreground">
        ...
        <p>{isDesktopRuntime ? '桌面端浏览器捕获默认使用本地页面解析，不依赖此服务。此服务主要补充公开 URL、YouTube 字幕和社媒深度解析；它不代理登录态、受限网页或 PDF 文档。' : '内容解析服务用于公开网页正文、YouTube 字幕和社媒解析。它不代理登录态、受限网页或 PDF 文档；与 AI 渠道完全独立。'}</p>
      </div>

      <div className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2"><h2 className="text-sm font-medium">自有内容解析服务</h2>{isDesktopRuntime && <span className="rounded bg-muted px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">桌面端可选</span>}</div>
            <p className="mt-1.5 text-[12px] leading-5 text-muted-foreground">服务地址与令牌仅保存在当前设备，不会写入 Flow 或随画板导出。</p>
          </div>
