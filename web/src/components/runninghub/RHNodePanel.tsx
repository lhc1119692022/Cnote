...
import { Check, ChevronDown, Layers3, LoaderCircle, Play, Square } from 'lucide-react'
...
function RHMenuSelect({ label, value, options, disabled, onChange, width }: { label: string; value: string; options: Array<{ value: string; label: string }>; disabled?: boolean; onChange: (value: string) => void; width: string }) {
  const [open, setOpen] = useState(false)
  const selected = options.find(option => option.value === value)
  return <div className={`relative min-w-0 ${width}`} onPointerDown={event => event.stopPropagation()}>
    <button type="button" aria-haspopup="listbox" aria-expanded={open} aria-label={label} disabled={disabled} onClick={() => setOpen(current => !current)} className="flex h-9 w-full items-center gap-2 rounded-lg border border-border bg-card px-3 text-left text-xs text-foreground hover:bg-muted disabled:opacity-50"><span className="min-w-0 flex-1 truncate">{selected?.label || label}</span><ChevronDown className={`h-3.5 w-3.5 shrink-0 text-muted-foreground transition-transform ${open ? 'rotate-180' : ''}`} /></button>
    {open && <div role="listbox" aria-label={label} className="cnote-menu-surface absolute bottom-[calc(100%+8px)] left-0 z-50 max-h-64 w-full min-w-[150px] overflow-auto" onPointerDown={event => event.stopPropagation()}>{!options.length && <div className="px-3 py-2 text-xs text-muted-foreground">暂无可选项</div>}{options.map(option => <button key={option.value} type="button" role="option" aria-selected={option.value === value} data-active={option.value === value} className="cnote-menu-item" onClick={() => { onChange(option.value); setOpen(false) }}><span className="min-w-0 flex-1 truncate text-left">{option.label}</span>{option.value === value && <Check className="h-3.5 w-3.5 shrink-0 text-primary" />}</button>)}</div>}
  </div>
}
...
export function RHNodePanel({ node }: { node: RequestNodeSpec }) {
  ...
  const selectWorkflow = (workflowId: string) => {
    ...
    const nextSelection = existing || { workflow: structuredClone(workflow), values: Object.fromEntries(workflow.fields.map(field => [field.key, field.value])), bindings: Object.fromEntries(workflow.fields.filter(field => ['image', 'video', 'audio'].includes(field.inputType)).map((field, index) => { const candidate = references.filter(item => item.type === field.inputType)[index]; return [field.key, candidate?.id || ''] })) }
  ...
  const configured = Boolean(channel && selected)
  useEffect(() => {
    const onToolbarAction = (event: Event) => {
      const detail = (event as CustomEvent<{ nodeId?: string; action?: string }>).detail
      if (detail?.nodeId !== node.id || detail.action !== 'update' || !selected || !latestDefinition || latestDefinition.revision === selected.revision || locked) return
  ...
  const missingMediaFields = mediaFields.filter(field => field.required && !selection?.bindings[field.key])
  return <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden p-3" onPointerDown={event => event.stopPropagation()} onWheel={event => event.stopPropagation()}>
    <div className="flex items-center gap-2 text-sm font-medium"><Layers3 className="h-4 w-4 text-primary" />RH 工作流</div>
    ...
    {selected && <div className="space-y-3 border-t border-border pt-3">
      {visibleFields.filter(field => ['image', 'video', 'audio'].includes(field.inputType)).map(field => <div key={field.key} className="space-y-1 text-xs text-muted-foreground"><label>{field.label}{field.required ? ' *' : ''}</label>
        <div className="flex flex-wrap gap-1.5">
          {references.filter(item => item.type === field.inputType).map(item => <button key={item.id} type="button" disabled={locked} onClick={() => patchSelection({ bindings: { ...selection?.bindings, [field.key]: item.id } })} className={`flex max-w-full items-center gap-1.5 rounded-lg border px-2 py-1.5 text-[11px] ${selection?.bindings[field.key] === item.id ? 'border-primary bg-primary/10 text-foreground' : 'border-border bg-background text-muted-foreground hover:border-primary/50'}`} title={item.label || item.id}>{item.previewUrl || item.url ? <img src={item.previewUrl || item.url} alt="" className="h-8 w-8 shrink-0 rounded object-cover" draggable={false} /> : <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded bg-muted text-[9px]">{field.inputType === 'image' ? '图' : field.inputType === 'video' ? '视' : '音'}</span>}<span className="truncate">{item.label || item.id}</span></button>)}
          {!references.some(item => item.type === field.inputType) && <span className="rounded-lg border border-dashed border-border px-2 py-1.5 text-[11px] text-muted-foreground">等待{field.inputType === 'image' ? '图片' : field.inputType === 'video' ? '视频' : '音频'}输入</span>}
        </div>
        <label className="inline-flex cursor-pointer items-center rounded-lg border border-dashed border-border px-2.5 py-1.5 text-[11px] text-muted-foreground hover:border-primary/50 hover:text-foreground"><span>添加{field.inputType === 'image' ? '图片' : field.inputType === 'video' ? '视频' : '音频'}</span><input type="file" aria-label={`上传 ${field.label}`} accept={`${field.inputType}/*`} disabled={locked} className="sr-only" onChange={async event => {
          const file = event.target.files?.[0]; event.target.value = ''
          if (!file) return
          try {
            if (!file.type.startsWith(`${field.inputType}/`)) throw new Error('素材类型不匹配')
            const asset = await new AssetManager().importAsset(file, file.name)
            const current = useGraphStore.getState().currentDocument?.nodes.find(item => item.id === node.id)
            if (!current || current.kind !== 'request' || current.rh?.workflowKey !== selected.id) { await new AssetManager().releaseAsset(asset.id); return }
            const config = current.rh
            useGraphStore.getState().updateNode(node.id, { rh: { ...config, referenceAssetIds: [...new Set([...config.referenceAssetIds, asset.id])], selections: { ...config.selections, [selected.id]: { ...config.selections[selected.id], bindings: { ...config.selections[selected.id].bindings, [field.key]: asset.id } } } } })
            useGraphStore.getState().commitHistory()
    ...
    <div className="flex shrink-0 items-center justify-end gap-3 border-t border-border pt-3">
      {!configured ? <>
        <RHMenuSelect label="RunningHub 渠道" value={rh.channelId || ''} width="w-[132px]" disabled={locked} options={channels.map(item => ({ value: item.id, label: item.name }))} onChange={selectChannel} />
        <RHMenuSelect label="选择工作流" value={rh.workflowKey || ''} width="w-[150px]" disabled={!channel || locked} options={available.map(item => ({ value: item.id, label: item.name }))} onChange={selectWorkflow} />
      </> : <div className="mr-auto flex min-w-0 items-center gap-2 text-xs text-muted-foreground"><span className="max-w-[132px] truncate">{channel?.name}</span><span className="text-border">/</span><span className="max-w-[150px] truncate">{selected?.name}</span></div>}
      {waiting ? <button type="button" className="h-9 shrink-0 rounded-lg border border-border px-2.5 text-xs" disabled={!task?.remoteTaskId} onClick={() => void resume()}>继续</button> : <button type="button" className="flex h-9 shrink-0 items-center gap-1.5 rounded-lg bg-foreground px-3 text-xs font-medium text-background shadow-sm hover:bg-foreground/85 disabled:opacity-50" disabled={!selected || locked || missingMediaFields.length > 0} onClick={() => void run()}>{running ? <LoaderCircle className="h-3.5 w-3.5 animate-spin" /> : <Play className="h-3.5 w-3.5" />}{running ? '运行中' : '运行工作流'}</button>}
    </div>
