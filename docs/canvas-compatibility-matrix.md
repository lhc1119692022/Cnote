...
- Selected the second split node via its border and clicked its native download action. The Save Cnote File dialog proposed `second.ico`. Saved to the previously absent `%TEMP%/cnote-media-export-20260917-acceptance.ico` without overwrite. Read-only verification: 13,843 bytes, exact Buffer equality with `desktop/packaging/resources/icon-cnote.ico`, SHA256 `d17833136eddcf73f72d28ed0a257ed2683de7c5c10e9a9fad1366b8187caa9d`. This proves current split-resource download after restart, not all media formats or remote-provider failures.
